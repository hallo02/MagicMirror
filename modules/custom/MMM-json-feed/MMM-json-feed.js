...
Module.register("MMM-json-feed", {
  ...
  getDom: function() {
    ...
    if (data && !this.isEmpty(data)) {
      ...
      var keyReplacement = this.config.keyReplacement;
      if (values.length > 0) {
        for (var i = 0; i < values.length; i++) {
          var val = this.getValue(data, values[i]);
          if (val) {
          	if (keyReplacement == null || keyReplacement.length == 0){
          		tableElement.appendChild(this.addValue(values[i], val));
            }else{
            	tableElement.appendChild(this.addValue(keyReplacement[i], val));
            }
          }
        }
      } else {
